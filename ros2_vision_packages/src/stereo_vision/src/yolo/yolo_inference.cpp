...
std::vector<stereo_vision_msgs::msg::Detection> performYOLOInference(const cv::Mat& image, rknn_app_context_t& app_ctx, float box_conf_threshold, float nms_threshold, rclcpp::Logger logger)
{
    ...
    if (ret < 0) {
        RCLCPP_ERROR(logger, "rknn_input_set fail! ret=%d", ret);
        return detections;
    }
    ...
    if (ret < 0) {
        RCLCPP_ERROR(logger, "rknn_run fail! ret=%d", ret);
        return detections;
    }
    ...
    if (ret < 0) {
        RCLCPP_ERROR(logger, "rknn_outputs_get fail! ret=%d", ret);
        return detections;
    }
    ...
    const float MIN_CONFIDENCE = 0.75f;  // Adjust this value as needed
    const int MIN_AREA = 150;  // Adjust this value as needed (e.g., 50x20 pixels)
